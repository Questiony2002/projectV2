...
import com.example.mental.dto.SCL90QuestionDTO;
...
public class SCL90Controller {
    ...
    public ResponseEntity<SCL90Result> getResultByUserId(@PathVariable Long userId) {
        ...
        if (resultOpt.isPresent()) {
            SCL90Result result = resultOpt.get();
            
            // 将JSON字符串转换为Map
            if (result.getFactorScoresJson() != null && !result.getFactorScoresJson().isEmpty()) {
                try {
                    // 使用Jackson将JSON字符串转换为Map
                    Map<String, Double> factorScores = objectMapper.readValue(
                            result.getFactorScoresJson(), 
                            new TypeReference<Map<String, Double>>() {});
                    
                    // 创建一个DTO用于传输，并设置转换后的Map
                    SCL90ResultDTO resultDTO = new SCL90ResultDTO();
                    resultDTO.setUserId(result.getUserId());
                    resultDTO.setTotalScore(result.getTotalScore());
                    resultDTO.setTotalAverage(result.getTotalAverage());
                    resultDTO.setPositiveItems(result.getPositiveItems());
                    resultDTO.setPositiveAverage(result.getPositiveAverage());
                    resultDTO.setFactorScores(factorScores);
                    
                    // 创建一个新的实体用于返回给客户端
                    SCL90Result responseResult = new SCL90Result();
                    responseResult.setId(result.getId());
                    responseResult.setUserId(result.getUserId());
                    responseResult.setTotalScore(result.getTotalScore());
                    responseResult.setTotalAverage(result.getTotalAverage());
                    responseResult.setPositiveItems(result.getPositiveItems());
                    responseResult.setPositiveAverage(result.getPositiveAverage());
                    responseResult.setFactorScoresJson(result.getFactorScoresJson());
                    
                    // 使用反射直接设置factorScores字段
                    try {
                        java.lang.reflect.Field field = SCL90Result.class.getDeclaredField("factorScores");
                        field.setAccessible(true);
                        field.set(responseResult, factorScores);
                    } catch (Exception e) {
                        // 如果反射失败，不影响返回结果，但会影响客户端的显示
                        e.printStackTrace();
                    }
